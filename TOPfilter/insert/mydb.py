...
class BaseDB(object):
    def __init__(self, database=None, user=None, passwd=None, host=None):
        ...
        self.host = host if host else '127.0.0.1:3306'
...
class Mydb(BaseDB):
    def __init__(self):
        BaseDB.__init__(self, database='deezer_meta')
    ...
    def insert_song(self, song, artist):
        self._execute('insert ignore meta(song, artist) '
                      'values (%s, %s)', (song, artist))
					  
	def insert_single(self, song):
        self._execute('insert ignore meta(song) '
                      'values (%s)', (song))				  
